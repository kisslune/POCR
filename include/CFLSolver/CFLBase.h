...
public:
    ...
    static void processArgs(int argc, char** argv, std::vector<std::string>& moduleNameVec)
    {
        for (int i = 0; i < argc; ++i) {
            std::vector<std::string> segs = SVFUtil::split(argv[i], '.');
            std::ifstream f(argv[i]);
            if (f.good()) {
                moduleNameVec.push_back(argv[i]);
            }
        }
    }
};

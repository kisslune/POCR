...
void CFLGraph::readGraph(std::string fname)
{
    ...
    while (getline(gFile, line)) {
        ...
        if (vec.size() == 4 && grammar->isVariantLabel(lbl))
...
void CFLGraph::writeGraph(std::string name)
{
    ...
    for (auto src: srcs) {
        ...
        for (auto edge: node->getOutEdges()) {
            ...
            if (grammar->isVariantLabel(edgeK))
                outFile << src << "\t" << dst << "\t" << grammar->getLabel(edgeK) << '\t' << edge->getEdgeIdx()
                        << std::endl;
            else
                outFile << src << "\t" << dst << "\t" << grammar->getLabel(edgeK) << std::endl;
        }
    }

...
void StdCFL::initialize()
{
    ...
    _grammar->readGrammarFile(grammarName);
...
Label StdCFL::binarySumm(Label lty, Label rty)
{
    ...
    if (grammar()->isVariantLabel(lty.first) &&
        grammar()->isVariantLabel(rty.first) &&
        lty.second == rty.second) {
        if (grammar()->isVariantLabel(lhs))
...
Label StdCFL::unarySumm(Label lty)
{
    ...
    if (grammar()->isVariantLabel(lhs) &&
        grammar()->isVariantLabel(lty.first))
        return std::make_pair(lhs, lty.second);
